/**
 * db/init.js — v5.0 (Turso Bulut Veritabanı Uyumlu)
 *
 * DEĞİŞENLER:
 * 1. sql.js ve WASM karmaşası tamamen kaldırıldı.
 * 2. Vercel'in sildiği /tmp dosyası yerine Turso (libsql) bulut bağlantısı eklendi.
 * 3. get, run, all, exec yardımcı fonksiyonları Turso execute mantığına uyarlandı.
 */
...
export async function initDb() {
  console.log("⚡ Turso bulut veritabanına bağlanılıyor...");
  await initSchema();
  await migrate();
  await seedIfEmpty();
  return db;
...
export function getDb() {
  if (!db) throw new Error('DB bağlantısı kurulamadı. initDb() çağırın.');
  return db;
}

/* ─── Query yardımcıları (Turso Uyumlu) ─────────────────────── */
export async function all(sql, params = []) {
  try {
    const result = await db.execute({ sql, args: params });
    return result.rows;
  } catch (error) {
    console.error(`❌ DB Hatası (all): ${sql}`, error);
    return [];
  }
}
...
export async function get(sql, params = []) {
  try {
    ...
    console.error(`❌ DB Hatası (get): ${sql}`, error);
    return null;
  }
}
...
export async function run(sql, params = []) {
  try {
    ...
    console.error(`❌ DB Hatası (run): ${sql}`, error);
...
export async function exec(sql) {
  try {
    // Turso'da çoklu SQL cümlelerini çalıştırmak için executeMultiple kullanılır
    await db.executeMultiple(sql);
  } catch (error) {
    console.error(`❌ DB Hatası (exec): ${sql}`, error);
  }
}

/* ─── Migration ─────────────────────────────────────────────── */
async function migrate() {
  const safeAdd = async (t, col, def) => { 
    try { await exec(`ALTER TABLE ${t} ADD COLUMN ${col} ${def}`) } catch {} 
  }
  
  await safeAdd('users', 'role', "TEXT DEFAULT 'user'");
  await safeAdd('users', 'phone', 'TEXT');
  await safeAdd('restaurants', 'owner_id', 'INTEGER REFERENCES users(id) ON DELETE SET NULL');
  await safeAdd('restaurants', 'status', "TEXT DEFAULT 'approved'");
  await safeAdd('restaurants', 'capacity', 'INTEGER DEFAULT 50');
  await safeAdd('restaurants', 'long_desc', 'TEXT');
  await safeAdd('restaurants', 'phone', 'TEXT');
  await safeAdd('restaurants', 'address', 'TEXT');
  await safeAdd('restaurants', 'hours', "TEXT DEFAULT '{}'");
  await safeAdd('restaurants', 'features', "TEXT DEFAULT '[]'");
  ...
  `);
  await exec(`UPDATE restaurants SET status='approved' WHERE status IS NULL`);
...
async function seedIfEmpty() {
  const restCount = await get('SELECT COUNT(*) as c FROM restaurants');
  
  if (restCount && restCount.c > 0) {
      console.log('✅ Veritabanı zaten dolu, Seed atlandı.');
      return;
  }
  
  console.log('🌱 Seed başlıyor... Turso bulut veritabanına örnek veriler yazılıyor.');
  ...
  for (const r of rests) {
    await run(`INSERT INTO restaurants (name,location,district,city,price_level,rating,review_count,description,long_desc,phone,address,hours,images,badges,features,is_featured,status,capacity) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
  ...
  for (const [rid,name,av,rating,text,vt] of reviews) {
    await run(`INSERT INTO reviews (restaurant_id,author_name,author_avatar,rating,text,visit_type) VALUES (?,?,?,?,?,?)`,
  ...
  for (const [title,slug,excerpt,body,cover_url,category,author_name,author_avatar,read_minutes] of blogs) {
    await run(`INSERT INTO blog_posts (title,slug,excerpt,body,cover_url,category,author_name,author_avatar,read_minutes) VALUES (?,?,?,?,?,?,?,?,?)`,
      [title,slug,excerpt,body,cover_url,category,author_name,author_avatar,read_minutes]);
  }

  // --- Şifreler hashlenerek eklenir ---
  const demoHash = bcrypt.hashSync('demo1234', 10);
  const adminHash = bcrypt.hashSync('admin1234', 10);
  const ownerHash = bcrypt.hashSync('owner1234', 10);

  // Turso'da SQLite gibi INSERT OR IGNORE çalışır.
  await run(`INSERT OR IGNORE INTO users (name,email,password,avatar,role) VALUES (?,?,?,?,?)`,['Demo Kullanıcı','demo@gastrovibe.com', demoHash, '🍽️','user']);
  await run(`INSERT OR IGNORE INTO users (name,email,password,avatar,role) VALUES (?,?,?,?,?)`,['Admin',         'admin@gastrovibe.com', adminHash,'👑','admin']);
  await run(`INSERT OR IGNORE INTO users (name,email,password,avatar,role) VALUES (?,?,?,?,?)`,['Demo Sahip',    'owner@gastrovibe.com', ownerHash,'🏪','owner']);

  console.log('✅ Seed: 10 restoran · 5 yorum · 4 blog · 3 kullanıcı Turso\'ya aktarıldı.');
}